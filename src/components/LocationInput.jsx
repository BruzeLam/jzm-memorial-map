...
export default function LocationInput({ value, onChange, onSelect, placeholder, inputClass }) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      />
      {showDropdown && (
        <div className="absolute z-[10000] left-0 right-0 top-full mt-0.5 bg-memorial-surface border border-memorial-border rounded-lg shadow-xl max-h-52 overflow-y-auto">
          {loading && (
            <div className="px-3 py-2 text-xs text-memorial-muted/70 flex items-center gap-2">
              <span className="inline-block w-3 h-3 border-2 border-memorial-gold border-t-transparent rounded-full animate-spin" />
              搜索中…
            </div>
          )}
          {!loading && results.map((item) => {
            ...
            return (
              <button
                ...
                className="w-full text-left px-3 py-2 hover:bg-amber-50 border-b border-memorial-border/60 last:border-0"
              >
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-memorial-ink truncate flex-1">{shortName}</span>
                  {item.source === 'amap' && (
                ...
                {region && (
                  <div className="text-xs text-memorial-muted/70 truncate">{region}</div>
                )}
              </button>
            );
          })}
