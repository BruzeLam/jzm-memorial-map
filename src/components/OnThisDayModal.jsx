...
function OnThisDayCard({ marker, markerTypeLabel, onSelectLabel, onSelect, onViewImages }) {
  ...
  return (
    <article className="rounded-xl border border-amber-100 bg-white shadow-sm overflow-hidden">
      {cover?.data ? (
        <button
          type="button"
          className="block w-full aspect-[4/3] bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500"
        ...
        <div
          className="w-full aspect-[2/1] flex items-center justify-center text-4xl"
      ...
      <div className="p-4 space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {marker.date && (
            <span className="text-xs text-amber-800/80 tabular-nums">
              {marker.date}
        ...
        <h3 className="text-base font-bold text-gray-900 leading-snug">{marker.name}</h3>
        {marker.title && <p className="text-sm font-medium text-gray-700">{marker.title}</p>}
        {region && <p className="text-xs text-gray-500">🌍 {region}</p>}
        {marker.description && (
          <p className="text-sm text-gray-600 leading-relaxed line-clamp-4">{marker.description}</p>
        )}
          ...
          <div className="flex gap-2 pt-1 overflow-x-auto">
            {marker.images.slice(1, 4).map((img, idx) => (
              <button
                key={idx}
                type="button"
                className="w-16 h-16 flex-shrink-0 rounded-lg overflow-hidden border border-gray-200"
            ...
            {marker.images.length > 4 && (
              <span className="text-xs text-gray-400 self-center">+{marker.images.length - 4}</span>
            )}
          </div>
        )}
        ...
        <button
          ...
          className="mt-1 text-xs font-medium text-amber-800 hover:text-amber-950 underline underline-offset-2"
        >
          {onSelectLabel}
        </button>
      </div>
...
export default function OnThisDayModal({ markers, onClose, onViewOnMap, onSelectMarker }) {
  ...
  return (
    <>
      ...
      >
        <div
          className="flex flex-col w-full max-w-md max-h-[min(720px,92vh)] rounded-2xl shadow-2xl overflow-hidden bg-gradient-to-b from-amber-50 to-white border border-amber-200/80"
          onClick={(e) => e.stopPropagation()}
          role="dialog"
          aria-labelledby="on-this-day-title"
        >
          <header className="flex-shrink-0 px-5 pt-5 pb-3 border-b border-amber-100/80">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold tracking-wide text-amber-700 uppercase">
                  📜 {t('onThisDayModal.eyebrow')}
                </p>
                <h2 id="on-this-day-title" className="text-xl font-serif font-bold text-gray-900 mt-0.5">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-4 space-y-4 on-this-day-scroll">
            {sorted.map((marker) => (
              <OnThisDayCard
                key={marker.id}
                marker={marker}
          ...
          <footer className="flex-shrink-0 px-4 py-3 border-t border-amber-100 bg-amber-50/60 flex gap-2">
            <button
              type="button"
              onClick={() => {
                onViewOnMap();
              ...
              className="flex-1 py-2.5 rounded-xl bg-amber-700 hover:bg-amber-800 text-white text-sm font-medium transition-colors"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2.5 rounded-xl border border-amber-200 bg-white hover:bg-amber-50 text-amber-900 text-sm font-medium transition-colors"
            >
