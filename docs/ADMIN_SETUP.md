...
2. **Authentication → URL Configuration**：
   - **Site URL**：`https://<your-app>.vercel.app`（当前实例见 [README](../README.md)）
...
### ⚠️ `onboarding@resend.dev` 与收件人

`onboarding@resend.dev` 是 Resend **测试发件地址**，可以发到 QQ、Gmail 等常见邮箱（你这边 QQ 能收到即说明已走通），但仍受 **100 封/天、3000 封/月** 限制。

朋友若收不到，常见原因 **不是**「未在 Resend 注册」：

1. **学校/企业邮箱拦截**（如 `@mail.ustc.edu.cn` 对外部魔法链接较严）→ 让朋友查垃圾箱，或换 Gmail/QQ 试  
2. **Supabase 发信频率**（项目约 50 封/小时）→ 等 30～60 分钟或复用未过期链接  
3. **Resend 日配额用尽**（100 封/天）→ 看 Resend → Usage / Logs  
4. **Supabase SMTP 配置异常** → Authentication → Logs

上线给更多人用时，仍建议在 Resend **验证自有域名**，发件人改为 `no-reply@你的域名`，进箱率更高。
