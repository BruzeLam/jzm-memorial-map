...
> **文档定位**：描述**可复用的产品架构与规格**，与具体历史人物解耦。本 GitHub 仓库为某一人物的**部署实例**；代码内目录名、localStorage 前缀等仍保留实例标识，见各章脚注。
...
| **最后更新** | 2026-06-03 |
| **仓库** | [BruzeLam/jzm-memorial-map](https://github.com/BruzeLam/jzm-memorial-map)（当前实例） |
| **默认分支** | `main` |
| **部署** | Vercel（push 自动部署） |
...
**历史人物纪念地图** 是一款以交互式地图为主线的纪念 / 教育向 Web **架构**。它将某一历史人物的**足迹、历史事件、题字地点**与**影像、语录、档案文献**串联在同一空间与时间维度上，帮助访客理解其一生的地理轨迹与历史脉络。Fork 后替换内置数据与品牌即可复用于其他人物。
...
历史人物的生平资料分散在地图坐标、新闻报道、讲话摘录、题字影像等多处。普通访客难以：
...
| I-03 | 纪念日倒计时 | P2 | 可选；实例配置重要日期后在顶栏展示倒计时 |
...
| C-02 | 彩蛋 | P2 | 可选；实例可在更新日志中嵌入产品历程类彩蛋 |
...
**存储键：** `{prefix}_memorial_markers`（localStorage；当前实例代码中为 `jzm_*`）  
...
**存储键：** `{prefix}_gallery_images`（当前实例：`jzm_gallery_images`）  
...
**运行时存储：** `{prefix}_all_quotes`（localStorage；当前实例：`jzm_all_quotes`）
...
| **Agent-III** | 风格化寄语 | 「以该人物文献语气鼓励后辈」 | 公开或独立模式 | 仅生成文本；事实必须可溯源 |
...
| v0.3 | 2026-06-03 | GitHub 文档脱敏：架构与具体人物解耦，便于开源复用 |
